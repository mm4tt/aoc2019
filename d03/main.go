...
func main() {
	...
	moves1, moves2 := strings.Split(lines[0], ","), strings.Split(lines[1], ",")

	traverse := func(moves []string, f func(point)) {
		p := point{0, 0}
		for _, m := range moves {
			...
			case 'D':
				d = point{0, -1}
			default:
				panic(fmt.Sprintf("invalid move: '%s'", m))
			}
	...
	func () {
		...
		var min *point
		traverse(moves1, func(p point) {
			d[p] = true
		})
		traverse(moves2, func(p point) {
			if d[p] {
				if min == nil || min.Norm() > p.Norm() {
					min = &p
				}
			}
		})
		fmt.Println(min.Norm())
	}()
