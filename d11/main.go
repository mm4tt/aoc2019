	...
	f := func(start int) map[util.Point2D]int {
		...
		async.Process(func(e intcode.AsyncProcessEvent) {
			...
			case move:
				...
				outputMode = color
			default:
				panic(fmt.Sprint("invalid outputMode: %v", outputMode))
			}
		})
		return hull
	}
