...
func main() {
	...
	fmt.Println(dist[oxygenSystemP])
...
func traverse(move func(d Dir) Tile, board map[util.Point2D]int, returnPath *stack.Stack, p util.Point2D) {
	for _, d := range Dirs {
		fmt.Printf("p = %v, trying to move %s\n", p, DirS[d])
		n := p.Add(DirP[d])
		fmt.Printf("new_p = %v\n", n)
		if _, ok := board[n]; ok {
			fmt.Println("new_p already visited, continuing")
			// Visited already.
			continue
		}
		t := move(d)
		fmt.Printf("Moved. Tile there is %s\n", TileS[t])
		board[n] = int(t)
		fmt.Println(t)
		switch t {
		case EMPTY:
			fallthrough
		case OXYGEN_SYSTEM:
			returnPath.Push(DirR[d])
			traverse(move, board, returnPath, n)
		case WALL:
			continue
		}
	}
	if returnPath.Len() > 0 {
		d := returnPath.Pop().(Dir)
		fmt.Printf("Moving back into %s\n", DirS[d])
		move(d)
	}
}
...
var DirP = map[Dir]util.Point2D{
	NORTH: {0, -1},
	SOUTH: {0, 1},
	WEST:  {-1, 0},
	EAST:  {1, 0},
}
var DirR = map[Dir]Dir{
	NORTH: SOUTH,
	SOUTH: NORTH,
	WEST:  EAST,
	EAST:  WEST,
...
const (
	WALL          Tile = 0
	EMPTY         Tile = 1
	OXYGEN_SYSTEM Tile = 2
	START         Tile = 3
)

var drawMapping = map[int]rune{
	int(WALL):          '#',
	int(EMPTY):         ' ',
	int(OXYGEN_SYSTEM): 'X',
	int(START):         'S',
}
var TileS = map[Tile]string{
	WALL:          "WALL",
	EMPTY:         "EMPTY",
